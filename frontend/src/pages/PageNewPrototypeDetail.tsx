...
const PageNewPrototypeDetail: FC<ViewPrototypeProps> = ({}) => {
  ...
  const { data: fetchedPrototype, isLoading: isPrototypeLoading } =
    useCurrentPrototype()
  const [prototype, setActivePrototype, setActiveModel] = useModelStore(
    (state) => [
  ...
  const [savedSession, setSavedSession] = useState<NewPrototypeSession | null>(
    null,
  )
  // Active plugin tab in new-prototype flow
  const [newFlowActivePluginId, setNewFlowActivePluginId] = useState<
    string | null
  >(null)
  // Active builtin tab in new-prototype flow ('code' | 'dashboard' | 'overview' | 'journey' | null)
  const [newFlowActiveBuiltinKey, setNewFlowActiveBuiltinKey] = useState<
    string | null
  >(null)
  ...
  const effectiveSidebarPlugin: string | undefined =
    effectiveModel?.custom_template?.prototype_sidebar_plugin || undefined

  // Extract global tab style variant (from effective model)
  const effectiveTabsVariant: string | undefined =
    effectiveModel?.custom_template?.prototype_tabs_variant || undefined
  ...
  useEffect(() => {
    ...
    const firstCustom = tabs.find(
      (t) => !t.hidden && t.type === 'custom' && t.plugin,
    )
  ...
  useEffect(() => {
    ...
    setShowRt(['dashboard'].includes(tab || ''))
  }, [tab, prototypeTabs])
  ...
  const handlePreviewModelChange = useCallback(
    (id: string | null) => setPreviewModelId(id ?? ''),
    [],
  )
  ...
  if (isNewPrototypeFlow) {
    const newFlowTabs = getTabConfig(
      effectiveModel?.custom_template?.prototype_tabs,
    )
    const newFlowVisibleTabs = newFlowTabs.filter((t) => !t.hidden)
    const activeNewFlowSidebar =
      effectiveModel?.custom_template?.prototype_sidebar_plugin || undefined
    const hasPrototype = !!newFlowPrototype && !openResumeDialog
    const newFlowShowRt =
      hasPrototype &&
      (newFlowActiveBuiltinKey === 'code' ||
        newFlowActiveBuiltinKey === 'dashboard')

    // Local tab navigation for new-prototype flow (never changes URL)
    const handleNewFlowSetActiveTab = (
      targetTab: string,
      ...
      <div className="flex w-full h-full relative">
        ...
        <div className="flex flex-col flex-1 h-full min-w-0">
            ...
            <div className="flex shrink-0 items-center">
              <StagingTabButton
                ...
                title={
                  hasPrototype
                    ? 'View staging'
                    : 'Create a prototype to enable staging'
                }
              />
              {/* {isModelOwner && (
                                <DropdownMenu>
          ...
          <div className="flex flex-1 h-full overflow-hidden relative">
            ...
            >
              ...
              {newFlowVisibleTabs
                .filter(
                  (t): t is TabConfig & { plugin: string } =>
                    t.type === 'custom' && !!t.plugin,
                )
                .map((tabConfig) => {
              ...
              {!newFlowActivePluginId &&
                newFlowActiveBuiltinKey === 'code' &&
                hasPrototype && <PrototypeTabCode />}
              {!newFlowActivePluginId &&
                newFlowActiveBuiltinKey === 'dashboard' &&
                hasPrototype && <PrototypeTabDashboard />}
              {!newFlowActivePluginId &&
                newFlowActiveBuiltinKey === 'journey' &&
                hasPrototype && (
                  <PrototypeTabJourney prototype={newFlowPrototype!} />
                )}
              {!newFlowActivePluginId &&
                newFlowActiveBuiltinKey === 'staging' &&
                hasPrototype && (
                  <PrototypeTabStaging prototype={newFlowPrototype!} />
                )}
              {!newFlowActivePluginId &&
                newFlowActiveBuiltinKey === 'feedback' &&
                hasPrototype && <PrototypeTabFeedback />}
              {!newFlowActivePluginId &&
                (!newFlowActiveBuiltinKey ||
                  newFlowActiveBuiltinKey === 'overview') &&
                hasPrototype && (
                  <PrototypeTabInfo prototype={newFlowPrototype!} />
                )}
            </div>
        ...
        >
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-xl font-semibold text-da-primary-500">
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button
                onClick={() =>
                  navigate(
                    `/model/${savedSession!.modelId}/library/prototype/${savedSession!.prototypeId}`,
                  )
                }
              >
                Continue
