        ...
        if (Array.isArray(currentValue) && Array.isArray(targetValue)) {
          ...
          resultTarget[key] = {
            valueDiff: targetValue.map((i, index) => [
              ...
              `${String(i) + (index === targetValue.length - 1 ? '' : ', ')}`,
            ]),
          }
          continue
        }
    ...
    >
      <div className="w-full h-full hidden">
        ...
        <select
          ...
          className={`w-[220px] ml-2 border rounded font-semibold text-center px-2 py-1 min-w-[90px] text-foreground bg-white`}
      ...
      <div className="flex-1 h-full w-full flex flex-col">
        <div className="bg-slate-100 p-2 flex items-center">
          ...
          <select
            aria-label="deploy-select"
            className="w-[280px] cursor-pointer transition border rounded font-semibold text-center text-sm px-2 py-1 min-w-[90px] text-foreground bg-white shadow-sm"
            value={activeCurrentVer}
            onChange={(e) => {
              setActiveCurrentVer(e.target.value)
            }}
          ...
          <select
            aria-label="deploy-select"
            className="w-[280px] cursor-pointer transition border rounded font-semibold text-sm text-center px-2 py-1 min-w-[90px] text-foreground bg-white shadow-sm"
