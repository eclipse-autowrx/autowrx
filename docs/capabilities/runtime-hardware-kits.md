...
- **Access token in preview URL:** `?access_token=` can appear in logs/Referer and is readable by kit JS via `location` on the first navigation. *Mitigation:* token is stripped before proxying to the kit; prefer short-lived access JWTs; the outer widget rejects `runtime-info` from non-same-origin sources (sandboxed kit posts as `"null"`) so kit content cannot retarget the preview via spoofed postMessage.
