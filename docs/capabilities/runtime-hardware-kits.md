...
Authenticated users only on HTTP — unauthenticated requests to any `/runtime-preview` path receive 401. Default JWT deployments authenticate the preview iframe via `access_token` query (posted from the parent dashboard; the widget only accepts `runtime-info` from `window.location.origin`). A Path-scoped HttpOnly `runtime_preview_at` cookie is also set on that first load as a best-effort aid for follow-up requests; under the opaque-origin sandbox (`allow-same-origin` omitted), `SameSite=Lax` cookies often are **not** sent on subresources initiated inside the kit iframe — relative assets that need auth may still 401. Primary flows rely on the URL token for the document navigation and on auth-exempt WS upgrades. `AUTH_PROVIDER=platform` continues to use injected identity headers. `pathFilter` confines `ws: true` upgrades to `/runtime-preview/*` (Express mount alone does not scope the server upgrade listener). WS upgrades still skip Express `auth()`; the proxy resolves the runtime name from `req.url` on upgrade and **destroys the socket** when the name/mapping is missing (fail closed — no default-target tunnel). CSP `frame-src` / `connect-src` must allow the preview path.
