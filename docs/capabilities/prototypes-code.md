...
- When an **owner** picks Copy Prototype from a prototype card's context or ⋮ menu at **Prototype library (`/model/:id/library`)**, they land on `/new-prototype?model_id=…&prototype_id=…` with the name pre-filled as "<source> (Copy)"; on submit the new prototype receives the source's content — code, language, APIs, widget config, images, customer journey, description, tags, complexity, portfolio, requirements and flow — and they are navigated to its detail page.
...
- `?prototype_id` query param → copy mode: the source is read via `GET /v2/prototypes/:id`, and after creation its content is written to the new prototype via `PATCH /v2/prototypes/:id`. Copied: `apis`, `code`, `language`, `widget_config`, `image_file`, `journey_image_file`, `analysis_image_file`, `customer_journey`, `description`, `tags`, `complexity_level`, `portfolio`, `skeleton`, `related_ea_components`, `partner_logo`, `requirements`, `requirements_data`, `flow`, `autorun`, plus `extend` with `copy: true`. Not copied: identity (`id`, `name`, `model_id`, `created_by`, timestamps), `state` (a copy starts in development), and usage/curation counters (`executed_turns`, `rated_by`, `last_viewed`, `editors_choice`). `extend.dashboard_template_id` is deliberately dropped so the copy's simulation tab loads the copied widget config rather than the template.
