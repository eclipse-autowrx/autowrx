...
- When an **owner** picks Copy Prototype from a prototype card's context or ⋮ menu at **Prototype library (`/model/:id/library`)**, they land on `/new-prototype?model_id=…&prototype_id=…` with the name pre-filled as "<source> (Copy)"; on submit the new prototype receives the source's code, APIs, widget config and cover image, and they are navigated to its detail page.
