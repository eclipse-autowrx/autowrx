...
const ProjectEditor: React.FC<ProjectEditorProps> = ({ data, onChange }) => {
  ...
  const handleContentChange = useCallback((file: File, content: string) => {
  ...
  const handleDeleteItem = (item: FileSystemItem) => {
    ...
    const newOpenFiles = openFiles.filter((openFile) => !filesToDelete.includes(openFile.name));
    setOpenFiles(newOpenFiles);
    ...
    if (creatingAtRoot && newRootItemName.trim()) {
      ...
      if (root && root.type === 'folder') {
        ...
        if (root.items.some(item => item.name === name)) {
          alert(`${creatingAtRoot.type} with name "${name}" already exists at the root.`);
          return;
        }

        const newItem: FileSystemItem = creatingAtRoot.type === 'file'
        ...
        setFsData(prevFsData => {
          ...
          if (rootItem && rootItem.type === 'folder') {
            const newRoot: Folder = { ...rootItem, items: [...rootItem.items, newItem] };
            newFsData[0] = newRoot;
            return newFsData;
          }
          return prevFsData;
            ...
            <div className="overflow-y-auto overflow-x-visible">
              ...
              {creatingAtRoot && (
                <div className="flex items-center py-[1px] px-2 text-gray-700 text-[13px] border-b border-gray-100">
                  <form onSubmit={handleRootCreateSubmit} className="w-full flex items-center">
                    {creatingAtRoot.type === 'folder' ? (
                      <VscNewFolder className="mr-2 text-gray-500 flex-shrink-0" size={16} />
                    ) : (
                      <VscNewFile className="mr-2 text-gray-500 flex-shrink-0" size={16} />
                    )}
                    <input
                      type="text"
