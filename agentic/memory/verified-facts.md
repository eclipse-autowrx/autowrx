...
- **Frontend dev port is 3210**, set by the `dev` script (`vite --port 3210`); not 3000/5173. (Source: `frontend/package.json`.)
...
- **ECA check is email-bound:** commits must be signed off (`git commit -s`) with the same email the author used to sign the Eclipse Contributor Agreement; the `eclipsefdn/eca` PR check fails otherwise. Identity is per-contributor, not repo-wide. (Source: `docs/getting-started/contributing.md` + `.github/workflows/` ECA check.)
