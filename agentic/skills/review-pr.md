...
6. **Ask before posting.** Present the verdict and findings to the operator first — posting to the PR is a separate, explicitly approved action. Never auto-post after a review; the operator's go for that specific post (not a standing habit, not a prior PR's approval) is required. On approval, post: verdict first; findings with severity + `file:line` + a concrete minimal fix; credit what genuinely works; tag the author when action is needed. If runtime verification overturns your own approval, post a correction that explicitly supersedes it — the thread must never end on a stale approval.
...
## Guardrails
- **Posting gate:** reviews are presented, not posted — every PR comment/review/correction goes out only after the operator approves that specific post.
